    ...
    <body>
        ...
        <div class="row register-form">
            <div class="col-md-8 offset-md-2">
                ...
                <?php
                    $client = "SELECT requestnc.* , client.id AS cltid , client.mail AS cltmail , client.lastname AS cltln , client.firstname AS cltfn , client.accountnumber AS cltan
                    FROM requestnc 
                    INNER JOIN client 
                    ON requestnc.idnewclient = client.id 
                    WHERE requestnc.idbanker = $bankerid AND requestnc.askdelete = 1 AND requestnc.delete = 0";
                    $reponse = $conn->query($client);
                    if ($reponse->num_rows > 0) 
                    {
                        while ($donnees = $reponse->fetch_assoc())
                        { 
                            $path = substr(base64_decode($donnees['pjdelete']),4);  ?>
                        ...
                        <div class="card cardbanker">
                            <div class="cardbanker card-body " style="border-style: solid;border-radius: 20px;">
                                ...
                                <h6 class="text-muted card-subtitle mb-2"><?php echo $donnees['cltadd']?></h6>
                                ...
                                <div style="text-align: right; color: blue;">
                                    <a href="./composent/database/validdelete.php?idclient=<?php echo $donnees['cltid']?>&token=<?php echo $token ?>&account=<?php echo $donnees['cltan']; ?>">Supprimer</a>
                                </div>
                            </div>
                        </div>
                <?php
